var page = {	
	init: function(){
		var OSM_layer, polygonLayer, mapOptions, constant;
		mapOptions = {
		...
		map.addLayers([constant.MAP_TILES, page.forecastLayer]);
		map.zoomToMaxExtent();
		...
		hoverControl = new OpenLayers.Control.SelectFeature(page.forecastLayer, {
			hover: true,
			highlightOnly: true,
			...
			for (b = 0; b < location.periods.length; b++){
				...
				if (period.id === period_id){
					for (c = 0; c < period.detail.length; c++){
						...
						page.forecastLayer.addFeatures([new OpenLayers.Feature.Vector(marker, attributes, markerStyle)]);
					}
				}
			}
		}
